...
/**
 * RestFUL HTTP请求 RestTemplateConfig restTemplate 使用方式：
 * 类名上面注解@Import({RestTemplateConfig.class}) 然后注入--注入是和普通一样的
 *
 * @author xy
 * @Autowired private RestTemplate restTemplate;
 * <p>
 * 依赖： <!--不带版本号 boot默认 http 请求 用于非RPC调用远程接口 这里主要用于restTemplate注入 -->
...
public class RestTemplateConfig {
    ...
    public RestTemplate getRestTemplate() {
        ...
        // 加入FastJson转换器
        converterList.add(new FastJsonpHttpMessageConverter4());
        return restTemplate;
    }
